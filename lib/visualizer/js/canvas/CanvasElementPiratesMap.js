...
CanvasElementPiratesMap.prototype.drawPirates = function() {
	...
	for (var hash in this.drawStates) {
		...
		for (n = drawList.length - 1; n >= 0; n--) {
			...
			//var hasMovePowerup = kf.pirate.movePowerupHistory[this.turn - Math.round(kf.pirate.time) - 1];
...
*/
				...
				if (this.state.config['showRange'] && kf['cloaked'] === 1) {
					...
					this.ctx.arc(kf.mapX + this.scale / 2, kf.mapY + this.scale / 2, attackRadiusShowRange, 0, 2 * Math.PI, false);
					this.ctx.fill();
				}
			} else {
				var w = this.scale;
...
CanvasElementPiratesMap.prototype.drawTreasures = function() {
	...
	for (var i = 0; i < treasures.length; i++) {
        ...
        var is_available_history = treasure[3];
...
CanvasElementPiratesMap.prototype.draw = function () {
	...
	this.ctx.drawImage(this.map.boardA, -130, -130);
	this.drawTreasures();
	this.drawPowerups();
	this.drawScripts();
